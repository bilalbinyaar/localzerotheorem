...
const BtcBalancePerExchange = (props) => {
      ...
      if (props.model_name.includes('collection')) {
        fetch(link + '/get/live_exchange', {
          ...
          .then((data) => {
            ...
            balance.push(
              parseFloat(data['response'][0].bitmex_btc_balance.toFixed(4))
            );
            balance.push(
              parseFloat(data['response'][0].okx_btc_balance.toFixed(4))
            );
...
export default BtcBalancePerExchange;
