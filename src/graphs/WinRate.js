    ...
    if (
      ...
      fetch(`https://zt-rest-api-rmkp2vbpqq-uc.a.run.app/${props.model_name}`, {
        ...
        .then(async (data) => {
          ...
          for (var index = 0; index < data["response"].length; index++) {
            if (
              (parseFloat(data["response"][index].win_rate) != 0 ||
                parseFloat(data["response"][index].win_rate) != -0) &&
              ...
              cum_pnl.push({
                ...
                y: parseFloat(data["response"][index].win_rate),
              });
            }
          }
      ...
      fetch(`https://zt-rest-api-rmkp2vbpqq-uc.a.run.app/get/live_pnls`, {
        ...
        .then(async (data) => {
          ...
          var cum_pnl_2 = [];
          for (var index = 0; index < data["response"].length; index++) {
            if (parseFloat(data["response"][index].win_rate)) {
              if (
                parseFloat(data["response"][index].win_rate) != 0 ||
                parseFloat(data["response"][index].win_rate) != -0
              ) {
          ...
          contentFormatter: (e) => {
            ...
            e.entries.forEach((entry) => {
              if (entry.dataPoint.smooth == false) {
                content += `<span style="color: black;">Win Rate: </span>${entry.dataPoint.y}<br/>`;
              } else {
